...
public class CombinedModel {
	...
	public static Double[][] skipFactors = new Double[OCRGlobals.numberOfChars][OCRGlobals.numberOfChars];
		...
		for(int i=0; i<imageIds.size()-1; i++) {
			for(int j=i+1; j<imageIds.size(); j++) {
				if(imageIds.get(i).equals(imageIds.get(j))) {
					int charId1 =  OCRGlobals.charsIndicesMap.get(chars.get(i));
					int charId2 = OCRGlobals.charsIndicesMap.get(chars.get(j));
					Double factor = skipFactors[charId1][charId2];
					score += Math.log(factor);
				}
			}
		}
	...
	public static Double wordWiseAccuracy(List<List<Integer>> images, List<List<Character> > words, List<List<Character> > predictedWords) {
		int correctlyPredicted = 0;
